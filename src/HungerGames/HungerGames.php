...
class HungerGames extends PluginBase implements Listener {
    ...
    public $prefix = TextFormat::YELLOW . "-" . TextFormat::AQUA . " ";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
	...
	{
            $this->getLogger()->info(TextFormat::GREEN . "HungerGames by thelucyclub enabled!");
        ...
        switch($cmd->getName()){
						...
						if($args[0]=="make")
						{
							if(!empty($args[1]))
							{
								if(file_exists($this->getServer()->getDataPath() . "/worlds/" . $args[1]))
								{
									...
									$this->mode = 1;
									$player->sendMessage($this->prefix . "Recording arena. Place blocks for spawn!");
									$player->setGamemode(1);
									$player->teleport($this->getServer()->getLevelByName($args[1])->getSafeSpawn(),0,0);
								}
								else
								{
									$player->sendMessage($this->prefix . "There is a world with that name.");
								}
							}
							else
							{
								$player->sendMessage($this->prefix . "Invalid parameters.");
							}
						}
						else
						{
							$player->sendMessage($this->prefix . "That is not a command.");
						}
					}
					else
					{
			...
			case "setrank":
				...
				if(!empty($args[0]))
				{
					...
					$config->save();
					$player->sendMessage($args[1] . " gained rank: " . $rank);
					}
					else
					{
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			if($this->mode==26)
			{
				$tile->setText(TextFormat::AQUA . "[Enter]",TextFormat::YELLOW  . "0 / 24",$this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
				$player->sendMessage($this->prefix . "Arena registrada correctamente!");
			}
			else
			{
				$text = $tile->getText();
				if($text[3] == $this->prefix)
				{
					...
					{
						$player->sendMessage($this->prefix . "You can not enter this game.");
					}
				}
			}
		}
		else if($this->mode>=1&&$this->mode<=24)
		{
			...
			if($this->mode==25)
			{
				$player->sendMessage($this->prefix . "Now it is the spawn of the final battle");
			}
			$config->save();
		}
		else if($this->mode==25)
		{
			...
			$player->sendMessage($this->prefix . "Returning to the lobby. Touch a sign to register the arena!");
			$spawn = $this->getServer()->getDefaultLevel()->getSafeSpawn();
			$this->getServer()->getDefaultLevel()->loadChunk($spawn->getFloorX(), $spawn->getFloorZ());
			$player->teleport($spawn,0,0);
			$config->save();
					...
					else
					{
						if(count($playersArena)>=2)
						{
							...
							else
							{
								...
								if(is_int($minutes) && $minutes>0)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage($this->prefix . $minutes . " minutes until deathmatch");
									}
								}
								else if($time2 == 300)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage($this->prefix . "The chests were reset!");
									}
									$this->refillChests($levelArena);
								}
								else if($time2 == 30 || $time2 == 15 || $time2 == 10 || $time2 ==5 || $time2 ==4 || $time2 ==3 || $time2 ==2 || $time2 ==1)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage($this->prefix . $time2 . " seconds until deathmatch.");
									}
								}
								if($time2 <= 0)
								{
								...
								else
								{
									...
									if($time <= 0)
									{
										...
										{
											$pl->teleport($spawn,0,0);
											$pl->sendMessage($this->prefix . "There are no winners this time!");
											$pl->getInventory()->clearAll();
										}
										$time = 780;
									}
						...
						else
						{
							...
							else
							{
								foreach($playersArena as $pl)
								{
									$pl->sendTip(TextFormat::RED . "Waiting for players. . .");
								}
								$config->set($arena . "PlayTime", 780);
								$config->set($arena . "StartTime", 60);
							}
